...
class Manejador_Materias:
    ...
    def __calcula_promedio(self, dni: str, lista1: list, lista2: list)->None:
        ...
        for materia in self.__lista:
            if materia.get_Dni()==dni:
                dni_encontrado=True
                if materia.get_nota() >= 4:
                    lista1.append(materia.get_nota())
                else:
                    lista2.append(materia.get_nota())
    ...
    def busca_promocional(self, materia:str)->List[Materia]:
        ...
        for row in self.__lista:
            if row.get_nom()==materia and row.get_nota()>=7 and row.get_Aproba()=="P":
               lista.append(row)
        return lista
